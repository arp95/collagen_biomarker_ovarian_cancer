...
masks = glob.glob(nuclei_masks + "*")
masks = = masks[:10000] 
for mask in masks:
    filename = mask.split("/")[-1]
    nuclei_mask = cv2.imread(mask, 0)
    macrophage_mask = cv2.imread(macrophage_masks + filename, 0)
    ...
    for cnt in cnts:
        ...
        count1 = 0
        for index1 in range(max(y-10, 0), min(y+h+10, 3000)):
            for index2 in range(max(x-10, 0), min(x+w+10, 3000)):
